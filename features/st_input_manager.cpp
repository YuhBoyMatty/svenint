...
DECLARE_HOOK( void, CALLCONV_CDECL, CmdStart, const edict_t *player, const usercmd_t *cmd, unsigned int random_seed );
...
ConVar st_im_autorecord( "st_im_autorecord", "0", FCVAR_EXTDLL, "Automatically record inputs at map start" );
ConVar st_im_autoplay( "st_im_autoplay", "0", FCVAR_EXTDLL, "Automatically play inputs at map start" );
ConVar st_im_exact( "st_im_exact", "0", FCVAR_EXTDLL, "Set origin & velocity" );
ConVar st_im_goto_exact( "st_im_goto_exact", "1", FCVAR_EXTDLL, "Set origin & velocity when forwarding / backwarding / goto" );
ConVar st_im_experimental( "st_im_experimental", "0", FCVAR_EXTDLL, "Experimental recording" );
ConVar st_im_experimental_play_save( "st_im_experimental_play_save", "0", FCVAR_EXTDLL, "Experimental recording, save into file all inputs during playback" );

static void st_im_record( const CCommand &args );
static void command_wrapper__st_im_record() { CCommand args( CVar()->ArgC(), CVar()->ArgV() ); st_im_record( args ); } \
static ConCommand st_im_record__command( "st_im_record", command_wrapper__st_im_record, "Record inputs to a file" ); \
static void st_im_record( const CCommand &args )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_record <filename>\n" );
	}
}

CON_COMMAND( st_im_play, "Playback inputs from a file" )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_play <filename>\n" );
	}
}

CON_COMMAND( st_im_split, "Split playing back inputs" )
{
	THIS_FEATURE()->Split();
}

CON_COMMAND( st_im_stop, "Stop recording inputs" )
{
	THIS_FEATURE()->Stop();
}

CON_COMMAND( st_im_goto, "Jump to X frame" )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_goto <frame>\n" );
	}
}

CON_COMMAND( st_im_forward, "Forward on X frames" )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_forward <frames>\n" );
	}
}

CON_COMMAND( st_im_backward, "Backward on X frames" )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_backward <frames>\n" );
	}
}

CON_COMMAND( st_im_frame, "Prints current frame" )
{
	Msg( "Frame: %d\n", THIS_FEATURE()->GetCurrentFrame() );
}

CON_COMMAND( st_im_test, "Prints current frame" )
{
...
static void st_im_record_command( const CCommand &args );
static void command_wrapper__st_im_record_command() { CCommand args( CVar()->ArgC(), CVar()->ArgV() ); st_im_record_command( args ); } \
static ConCommand st_im_record_command__command( "st_im_record_command", command_wrapper__st_im_record_command, "Record a given command" ); \
static void st_im_record_command( const CCommand &args )
{
	if ( args.ArgC() >= 2 )
	{
	...
	{
		Msg( "Usage:  st_im_record_command <command>\n" );
	}
}
...
DECLARE_FUNC( void, CALLCONV_CDECL, HOOKED_CmdStart, const edict_t *player, const usercmd_t *cmd, unsigned int random_seed )
{
	THIS_FEATURE()->OnCmdStart( const_cast<edict_t *>( player ), const_cast<usercmd_t *>( cmd ) );

	ORIG_CmdStart( player, cmd, random_seed );
}
...
void CInputContext::RecordInput( float frametime, usercmd_t *cmd, int active )
{
	...
	if ( st_im_experimental.GetBool() && THIS_FEATURE()->GetExperimental() != NULL )
	{
		edict_t *pPlayer;
		...
		{
			fprintf( THIS_FEATURE()->GetExperimental(), "%d:%.3f %.3f %.3f:%.3f %.3f %.3f:%.3f %.3f %.3f\n",
...
void CInputContext::PlaybackInput( float frametime, usercmd_t *cmd, int active )
{
	...
	if ( st_im_experimental_play_save.GetBool() && THIS_FEATURE()->GetExperimental() != NULL )
	{
		edict_t *pPlayer;
...
bool CInputManager::Record( const char *pszFilename )
{
	...
	gameutils->PrintChatText( "<SvenInt> Recording inputs to file \"%s\"\n", m_InputContext.FileName() );
...
bool CInputManager::Playback( const char *pszFilename )
{
	...
	gameutils->PrintChatText( "<SvenInt> Playing back inputs from file \"%s\"\n", m_InputContext.FileName() );
	...
	if ( st_im_experimental_play_save.GetBool() )
		m_pExperimental = fopen( SVENINT_FOLDER_NAME "/input_manager/experimental.txt", "w" );
...
bool CInputManager::Split( void )
{
	if ( m_state == IM_NONE )
	{
		PrintWarning( "Not playing back anything\n" );
		return false;
	}
	...
	gameutils->PrintChatText( "<SvenInt> Splitted inputs from file \"%s\" on frame %d. Recording now\n", m_InputContext.FileName(), m_InputContext.FrameCounter() + 1 );
...
bool CInputManager::Stop( bool bAutoStop /* = false */ )
{
	if ( m_state != IM_NONE )
	{
		...
		{
		#ifdef SINT_FEATURE_ST_ROUTE_EDITOR_H
			extern ConVar st_route_im_autoplay_mode;
			if ( st_route_im_autoplay_mode.GetInt() == 2 )
			{
				st_route_im_autoplay_mode.SetValue( 1 );
				gameutils->PrintChatText( "<SvenInt> Autoset value of st_route_im_autoplay_mode to 1 (Stop) for your safety\n" );
			}
		#endif

			PrintMsg( "Saved recorded inputs to file \"%s\" (frames: %d)\n", m_InputContext.FileName(), m_InputContext.Frames().size() );
			if ( !bAutoStop )
				gameutils->PrintChatText( "<SvenInt> Saved recorded inputs to file \"%s\" (frames: %d)\n", m_InputContext.FileName(), m_InputContext.Frames().size() );
		...
		else
		{
			...
			if ( !bAutoStop )
			{
				gameutils->PrintChatText( "<SvenInt> Stopped playing back inputs from file \"%s\" (frames: %d)\n", m_InputContext.FileName(), GetCurrentFrame() );
				Modules::scripts->Callbacks()->OnPlayEnd( m_InputContext.m_sFileName.c_str(), (int)m_InputContext.m_frames.size() );
			}
		}
	}
...
void CInputManager::Goto( int iFrame )
{
	...
	gameutils->PrintChatText( "<SvenInt> Jumped to frame %d\n", GetCurrentFrame() );
}
...
void CInputManager::Forward( int iFrames )
{
	...
	{
		gameutils->PrintChatText( "<SvenInt> Forwarded on %d frames. Current frame: %d\n", iFramesDifference, GetCurrentFrame() );
	}
}
...
void CInputManager::Backward( int iFrames )
{
	...
	if ( iFramesDifference > 0 )
	{
		gameutils->PrintChatText( "<SvenInt> Backwarded on %d frames. Current frame: %d\n", iFramesDifference, GetCurrentFrame() );
	}
}
...
//-----------------------------------------------------------------------------
// Record a given command / sequence of commands FOR THIS FRAME
//-----------------------------------------------------------------------------

void CInputManager::RecordCommandNow( const char *pszCommand )
{
	if ( m_state != IM_RECORDING )
	{
		//PrintWarning( "Not playing back anything\n" );
	...
	if ( pszCommand == NULL || Features::inputmanager->GetInputContext().FrameCounter() - 1 < 0 )
	{
		return;
	}
	...
	{
		frames[ prevframe ].commands = strdup( pszCommand );
	}
}
...
void CInputManager::CreateMove( float frametime, usercmd_t *cmd, int active )
{
	...
	{
		extern ConVar st_route_im_autoplay_mode;
		extern ConVar st_route_im_autoplay_tick;

		// We're done playing back inputs
		if ( m_InputContext.FrameCounter() >= (int)m_InputContext.Frames().size() - 1 )
		{
			Stop();
		}
	#ifdef SINT_FEATURE_ST_ROUTE_EDITOR_H
		else if ( st_route_im_autoplay_mode.GetBool() && st_route_im_autoplay_tick.GetInt() == m_InputContext.FrameCounter() )
		{
			int mode = st_route_im_autoplay_mode.GetInt();

			Features::routeeditor->Playback();

			if ( mode == 1 ) // Stop playing back
				Stop();
			else // Split and record
				Split();
		}
	#endif
	}
...
{
	if ( st_im_autorecord.GetBool() || st_im_autoplay.GetBool() )
	{
		if ( m_state == IM_NONE )
		{
...
{
	if ( m_state == IM_RECORDING && *pszCommand != '\n' )
	{
...
void CInputManager::OnCmdStart( edict_t *player, usercmd_t *cmd )
{
}
...
void CInputManager::PostLoad( void )
{
	...
	//m_hCmdStart = Detours()->DetourFunction( dllFuncs->pfnCmdStart, HOOKED_CmdStart, GET_FUNC_PTR( ORIG_CmdStart ) );
	...
	FEATURE_REGISTER_CVAR( st_im_autorecord );
	FEATURE_REGISTER_CVAR( st_im_autoplay );
	FEATURE_REGISTER_CVAR( st_im_exact );
	FEATURE_REGISTER_CVAR( st_im_goto_exact );
	FEATURE_REGISTER_CVAR( st_im_experimental );
	FEATURE_REGISTER_CVAR( st_im_experimental_play_save );
	FEATURE_REGISTER_CCMD( st_im_record_ );
	FEATURE_REGISTER_CCMD( st_im_play );
	FEATURE_REGISTER_CCMD( st_im_split );
	FEATURE_REGISTER_CCMD( st_im_stop );
	FEATURE_REGISTER_CCMD( st_im_goto );
	FEATURE_REGISTER_CCMD( st_im_forward );
	FEATURE_REGISTER_CCMD( st_im_backward );
	FEATURE_REGISTER_CCMD( st_im_frame );
	FEATURE_REGISTER_CCMD( st_im_test );
	FEATURE_REGISTER_CCMD( st_im_record_command_ );
}
...
{
	FEATURE_UNREGISTER_CVAR( st_im_autorecord );
	FEATURE_UNREGISTER_CVAR( st_im_autoplay );
	FEATURE_UNREGISTER_CVAR( st_im_exact );
	FEATURE_UNREGISTER_CVAR( st_im_goto_exact );
	FEATURE_UNREGISTER_CVAR( st_im_experimental );
	FEATURE_UNREGISTER_CVAR( st_im_experimental_play_save );
	FEATURE_UNREGISTER_CCMD( st_im_record_ );
	FEATURE_UNREGISTER_CCMD( st_im_play );
	FEATURE_UNREGISTER_CCMD( st_im_split );
	FEATURE_UNREGISTER_CCMD( st_im_stop );
	FEATURE_UNREGISTER_CCMD( st_im_goto );
	FEATURE_UNREGISTER_CCMD( st_im_forward );
	FEATURE_UNREGISTER_CCMD( st_im_backward );
	FEATURE_UNREGISTER_CCMD( st_im_frame );
	FEATURE_UNREGISTER_CCMD( st_im_test );
	FEATURE_UNREGISTER_CCMD( st_im_record_command_ );
	...
	//Detours()->RemoveDetour( m_hCmdStart );
