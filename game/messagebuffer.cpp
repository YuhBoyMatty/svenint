...
CMessageBuffer::CMessageBuffer( const char *pszName, int iBufferSize )
{
	m_pBuffer = &m_buffer;
	...
	m_buffer.buffername = pszName;
	m_buffer.flags = SIZEBUF_CHECK_OVERFLOW;
...
CMessageBuffer::CMessageBuffer( const char *pszName, void *buffer, int iSize, bool bBufferFilled /* = false */ )
{
	Init( pszName, buffer, iSize, bBufferFilled );
}

CMessageBuffer::CMessageBuffer( CNetMessageParams *netparams )
{
	Init( netparams );
}
...
void CMessageBuffer::Init( const char *pszName, void *buffer, int iSize, bool bBufferFilled /* = false */ )
{
	m_pBuffer = &m_buffer;
...
void CMessageBuffer::Init( CNetMessageParams *netparams )
{
	Init( netparams->buffer, netparams->readcount, netparams->badread );
}
...
bool CMessageBuffer::IsBufferAllocated( void ) const
{
	return m_bBufferAllocated;
}
...
void CMessageBuffer::WriteBuf( void *buffer, int size )
{
	...
	if ( m_pBuffer->flags & SIZEBUF_OVERFLOWED )
		return;
...
{
	m_pBuffer->cursize = 0;
	m_pBuffer->flags &= ~SIZEBUF_OVERFLOWED;
}
...
void *CMessageBuffer::GetSpace( int length )
{
	...
	{
		if ( !( m_pBuffer->flags & SIZEBUF_ALLOW_OVERFLOW ) )
			Warning( "[SvenInt] CMessageBuffer::GetSpace: overflow without allowoverflow set (%s) (%d)", m_pBuffer->buffername, m_pBuffer->maxsize );

		if ( length > m_pBuffer->maxsize )
			Warning( "[SvenInt] CMessageBuffer::GetSpace: %i is > full buffer size (%s)", length, m_pBuffer->buffername );

		Warning( "[SvenInt] CMessageBuffer::GetSpace: overflow (%s)\n", m_pBuffer->buffername );
		Clear();
