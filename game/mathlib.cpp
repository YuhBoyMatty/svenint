...
void VectorAngles( const float *forward, float *angles )
{
	...
	{
		yaw = VEC_RAD2DEG( atan2f( forward[ 1 ], forward[ 0 ] ) );
		if ( yaw < 0.f )
			yaw += 360.f;

		tmp = sqrtf( forward[ 0 ] * forward[ 0 ] + forward[ 1 ] * forward[ 1 ] );
		pitch = VEC_RAD2DEG( atan2f( forward[ 2 ], tmp ) );
		if ( pitch < 0.f )
			pitch += 360.f;
	}
...
void QuaternionSlerp( vec4_t p, vec4_t q, float t, vec4_t qt )
{
	...
	else
	{
		...
		qt[ 2 ] = -q[ 3 ];
		qt[ 3 ] = q[ 2 ];
		sclp = sinf( ( 1.0f - t ) * static_cast<float>( 0.5 * M_PI ) );
		sclq = sinf( t * static_cast<float>( 0.5 * M_PI ) );
		for ( i = 0; i < 3; i++ )
		{
