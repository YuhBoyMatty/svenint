...
#ifndef SINT_CONVAR_H
#define SINT_CONVAR_H
...
void ConVar_Register( int nCVarFlag = ( 1 << 4 ) /* FCVAR_CLIENTDLL */, IConCommandBaseAccessor *pAccessor = 0 );
...
class ConCommandBase
{
	...
	friend void ConVar_Unregister( void );

public:
	ConCommandBase( void );
	ConCommandBase( const char *pszName, const char *pszHelpString = 0, int flags = 0 );
...
public:
	...
	struct cmd_function_s *GetCmdPointer( void ) const;

private:
	struct cmd_function_s *m_pCommand;
...
public:
	...
	struct cvar_s				*GetCvarPointer( void ) const;
...
#define EXPAND_CON_COMMAND( name ) name##_command

#define CON_COMMAND( name, description ) \
	static void name( const CCommand &args ); \
	static void command_wrapper__##name() { CCommand args( Globals::cvar->ArgC(), Globals::cvar->ArgV() ); name( args ); } \
	static ConCommand name##_command( #name, command_wrapper__##name, description ); \
	static void name( const CCommand &args )

#define CON_COMMAND_CMDNAME( name, cmdname, description ) \
	static void name( const CCommand &args ); \
	static void command_wrapper__##name() { CCommand args( Globals::cvar->ArgC(), Globals::cvar->ArgV() ); name( args ); } \
	static ConCommand name##_command( cmdname, command_wrapper__##name, description ); \
	static void name( const CCommand &args )
...
#define CON_COMMAND_EXTERN( name, _funcname, description ) \
	...
	static void command_wrapper__##name() { CCommand args( Globals::cvar->ArgC(), Globals::cvar->ArgV() ); _funcname( args ); } \
	static ConCommand name##_command( #name, command_wrapper__##name, description ); \
	void _funcname( const CCommand &args )
...
#define CON_COMMAND_ARGS( name ) CCommand name( Globals::cvar->ArgC(), Globals::cvar->ArgV() )
