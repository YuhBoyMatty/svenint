...
public:
	...
	{
		Globals::cvar->RegisterConCommand( pVar );
		return true;
	}
};
...
void ConVar_Register( int nCVarFlag, IConCommandBaseAccessor *pAccessor )
{
	...
	s_nDLLIdentifier = Globals::cvar->AllocateDLLIdentifier();
...
void ConVar_Unregister( void )
{
	...
	Globals::cvar->UnregisterConCommands( s_nDLLIdentifier );
...
void ConVar_PrintDescription( const ConCommandBase *pVar )
{
	...
	if ( !pVar->IsCommand() )
	{
		...
		if ( var != NULL )
		{
			ConColorMsg( { 255, 100, 100, 255 }, "\"%s\" = \"%s\"", var->GetName(), pszValue );
	...
	{
		ConColorMsg( { 255, 100, 100, 255 }, "\"%s\"", pVar->GetName() );
	}

	const char *pszHelpText = pVar->GetHelpText();

	if ( pszHelpText != NULL && *pszHelpText )
	{
		ConMsg( "\n - %s\n", pszHelpText );
	}
	else
...
{
	if ( Globals::cvar != NULL )
	{
		Globals::cvar->RegisterConCommand( this );
	}
	//if ( s_pAccessor != NULL )
	//{
...
void ConCommandBase::Shutdown( void )
{
	if ( Globals::cvar != NULL )
	{
		Globals::cvar->UnregisterConCommand( this );
	}
}
...
cmd_function_t *ConCommand::GetCmdPointer( void ) const
{
	return m_pCommand;
}
...
Color ConVar::GetColor( void ) const
{
	if ( IsRegistered() && m_pCvar != NULL )
	{
		return Globals::cvar->GetColorFromCvar( m_pCvar );
	}
...
void ConVar::SetValue( const char *value )
{
	if ( IsRegistered() && m_pCvar != NULL )
	{
		Globals::cvar->SetValue( m_pCvar, value );
	}
}
...
void ConVar::Revert( void )
{
	if ( IsRegistered() && m_pCvar != NULL )
	{
		Globals::cvar->SetValue( m_pCvar, m_pszDefaultValue );
	}
}
...
bool ConVar::Clamp( void )
{
	if ( IsRegistered() && m_pCvar != NULL )
	{
		...
		{
			Globals::cvar->SetValue( m_pCvar, m_fMinVal );
			return true;
		}

		if ( m_bHasMax && ( value > m_fMaxVal ) )
		{
			Globals::cvar->SetValue( m_pCvar, m_fMaxVal );
			return true;
		}
	}
