...
DEFINE_SCRIPTFUNC( FindCvar )
{
	...
	lua_pushboolean( pLuaState, Globals::cvar->FindCvar( pszCvarName ) != NULL );
...
DEFINE_SCRIPTFUNC( FindCmd )
{
	...
	lua_pushboolean( pLuaState, Globals::cvar->FindCmd( pszCmdName ) != NULL );
...
{
	const char *pszCvarName = lua_tostring( pLuaState, 1 );
	cvar_t *pCvar = Globals::cvar->FindCvar( pszCvarName );
	...
	if ( lua_isinteger( pLuaState, 2 ) )
	{
		...
		Globals::cvar->SetValue( pCvar, value );
...
DEFINE_SCRIPTFUNC( GetFloat )
{
	...
	lua_Number dbValue = (lua_Number)Globals::cvar->GetFloatFromCvar( pCvar );
...
DEFINE_SCRIPTFUNC( GetInteger )
{
	...
	lua_Integer iValue = (lua_Integer)Globals::cvar->GetIntFromCvar( pCvar );
...
DEFINE_SCRIPTFUNC( GetString )
{
	...
	const char *pszValue = Globals::cvar->GetStringFromCvar( pCvar );
...
DEFINE_SCRIPTFUNC( GetBool )
{
	...
	bool bValue = Globals::cvar->GetBoolFromCvar( pCvar );
