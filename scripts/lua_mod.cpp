...
static const char *GetMapName()
{
	...
	strncpy( mapname_buffer, Globals::cl_enginefuncs->pfnGetLevelName(), MAX_PATH );
...
DEFINE_SCRIPTFUNC( ClientCmd )
{
	...
	Globals::cl_enginefuncs->pfnClientCmd( pszConCommand );
...
DEFINE_SCRIPTFUNC( AllocEngineString )
{
	...
	int string = Globals::sv_enginefuncs->pfnAllocString( pszString );
...
DEFINE_SCRIPTFUNC( KeyValue )
{
	...
	KeyValueData kv = { Globals::gpGlobals->pStringBase + pEdict->v.classname, pszKeyname, pszValue, 0 };

	Globals::dllFuncs->pfnKeyValue( pEdict, &kv );
...
DEFINE_SCRIPTFUNC( GetPEntityFromEntityIndex )
{
	...
	edict_t *pEdict = Globals::sv_enginefuncs->pfnPEntityOfEntIndex( iEntIndex );
...
DEFINE_SCRIPTFUNC( GetPEntityFromPlayerName )
{
	...
	{
		edict_t *pEdict = Globals::sv_enginefuncs->pfnPEntityOfEntIndex( i );

		if ( Modules::server->IsValidEntity( pEdict ) &&
			 !strcmp( pszPlayerName, Globals::gpGlobals->pStringBase + pEdict->v.netname ) )
		{
			pPlayer = pEdict;
			break;
		}
...
DEFINE_SCRIPTFUNC( GetEntityIndexFromEdict )
{
	...
	lua_pushinteger( pLuaState, (lua_Integer)Globals::sv_enginefuncs->pfnIndexOfEdict( pEdict ) );
...
DEFINE_SCRIPTFUNC( SendCommandToClient )
{
	...
	Globals::sv_enginefuncs->pfnMessageBegin( MSG_ONE, SVC_SVENINT, NULL, ed );
		Globals::sv_enginefuncs->pfnWriteByte( SVENINT_COMM_EXECUTE );
		Globals::sv_enginefuncs->pfnWriteString( command );
...
DEFINE_SCRIPTFUNC( SendSignalToClient )
{
	...
	Globals::sv_enginefuncs->pfnMessageBegin( MSG_ONE, SVC_SVENINT, NULL, ed );
		Globals::sv_enginefuncs->pfnWriteByte( SVENINT_COMM_SCRIPTS );
		Globals::sv_enginefuncs->pfnWriteByte( 0 ); // signal from server
		Globals::sv_enginefuncs->pfnWriteLong( value );
	Globals::sv_enginefuncs->pfnMessageEnd();
...
{
	lua_pushboolean( pLuaState, (int)Modules::server->Host_IsServerActive() );
...
{
	lua_pushboolean( pLuaState, (int)Modules::server->IsSurvivalModeEnabled() );
...
{
	lua_pushboolean( pLuaState, (int)Modules::server->EnableSurvivalMode() );
...
{
	lua_pushboolean( pLuaState, (int)Modules::server->DisableSurvivalMode() );
...
DEFINE_SCRIPTFUNC( SetTimescale )
{
	...
	Features::timescale->SetTimescale( timescale );
...
{
	lua_pushnumber( pLuaState, (lua_Number)Features::timer->SegmentCurrentTime() );
...
DEFINE_SCRIPTFUNC( TraceHull )
{
	...
	int traceFlags = (int)lua_tointeger( pLuaState, 4 );
	int hullType = (int)lua_tointeger( pLuaState, 5 );
	int ignoreEnt = (int)lua_tointeger( pLuaState, 6 );

	int oldhull = Globals::playermove->usehull();
	Globals::playermove->setusehull( hullType );

	pmtrace_t tr = Globals::playermove->funcs()->PM_PlayerTrace( (float *)vecStart, (float *)vecEnd, traceFlags, ignoreEnt );

	//pmtrace_t *tr = g_pPlayerMove->PM_TraceLine( (float *)vecStart, (float *)vecEnd, traceFlags, hullType, ignoreEnt );
	//pmtrace_t *tr = Globals::cl_enginefuncs->PM_TraceLine( (float *)vecStart, (float *)vecEnd, traceFlags, hullType, ignoreEnt );

	// FIXME
	//pmtrace_t tr;
	//tr = PM_PlayerTrace( (float *)vecStart, (float *)vecEnd, traceFlags, Globals::playermove->numphysent(), Globals::playermove->physents(), ignoreEnt, NULL );

	//g_pEventAPI->EV_SetTraceHull( hullType );
	//g_pEventAPI->EV_PlayerTrace( (float *)vecStart, (float *)vecEnd, traceFlags, ignoreEnt, &tr );

	Globals::playermove->setusehull( oldhull );
...
DEFINE_SCRIPTFUNC_NONSTATIC( Aimbot )
{
	...
	if ( bAimbot )
		iMode = 1;
	else if ( bSilentAimbot )
		iMode = 2;
	else if ( bRagebot )
		iMode = 3;

	bool bExecuted = Features::aim->Aimbot( cmd, iMode, bChangeAnglesBack, bAnglesChanged );

	lua_pushboolean( pLuaState, bExecuted );
...
LUALIB_API int luaopen_mod( lua_State *pLuaState )
{
	...
	// Max clients
	VLua::RegisterGlobalVariable( "MaxClients", Globals::gpGlobals->maxClients );

	// Client state
	VLua::RegisterGlobalVariable<int>( "CLS_NONE", ca_uninitialized );
	VLua::RegisterGlobalVariable<int>( "CLS_DISCONNECTED", ca_disconnected );
	VLua::RegisterGlobalVariable<int>( "CLS_CHALLENGE", ca_connecting );
	VLua::RegisterGlobalVariable<int>( "CLS_CONNECTED", ca_connected );
	VLua::RegisterGlobalVariable<int>( "CLS_LOADING", ca_uninitialized );
	VLua::RegisterGlobalVariable<int>( "CLS_ACTIVE", ca_active );

	VLua::RegisterGlobalVariable<int>( "ca_uninitialized", ca_uninitialized );
	VLua::RegisterGlobalVariable<int>( "ca_disconnected", ca_disconnected );
	VLua::RegisterGlobalVariable<int>( "ca_connecting", ca_connecting );
	VLua::RegisterGlobalVariable<int>( "ca_connected", ca_connected );
	VLua::RegisterGlobalVariable<int>( "ca_uninitialized", ca_uninitialized );
	VLua::RegisterGlobalVariable<int>( "ca_active", ca_active );
